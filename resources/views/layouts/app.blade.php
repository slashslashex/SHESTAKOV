...
<head>
    ...
    @elseif (request()->route()->getName() === 'signin')
        ...
        <div class="container d-flex flex-column flex-md-row align-items-center justify-content-between gap-2 gap-md-1">
            ...
            @if (Auth::check())
                ...
                <div class="d-flex flex-column align-items-center">
                    @if($user->points > 0)
                    <span class="bg-primary fs-6 text-light px-2 rounded-pill">{{ $user->points }} баллов</span>
                    @else
                    <span class="bg-primary fs-6 text-light px-2 rounded-pill">Скидочные баллы отстутствуют</span>
                    @endif
                    <span class="login text-light fs-6">{{ $user->email }} </span>
                </div>
            @else
                <div class="d-flex flex-column align-items-center">
                    <span class="login text-light fs-6">ВЫ НЕ АВТОРИЗОВАНЫ</span>
                </div>
            @endif
            <div>
                ...
                <a class="btn" href="{{ route('signin') }}" title="Войти">
                    <svg width="35px" height="35px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <!-- Кнопка выйти -->
                @if (Auth::check())
                        <a class="btn" href="{{ route('logout') }}" title="Выйти">
